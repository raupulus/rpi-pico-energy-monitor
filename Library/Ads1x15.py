...
class ADS1115Lib(object):
    __autoRangeMode = False
    __voltageRange = 2048
    __measureMode = ADS1115_SINGLE

    def __init__(self, i2c, address=__ADS1115_DEFAULT_ADDR):
        ...
        try:
            ...
            raise ValueError(
                "Can't connect to the ADS1115. Check wiring, address, etc.")
    ...
    def __calcLimit(self, rawLimit):
        limit = int((rawLimit * __ADS1115_REG_FACTOR /
                     self.__voltageRange) * 1000)
        if limit > 32767:
            limit -= 65536
        return limit
    ...
    def setSingleChannel(self, channel):
        if channel >= 4:
            return
        self.setCompareChannels(
            (ADS1115_COMP_0_GND + ADS1115_COMP_INC) * channel)
    ...
    def __bytesToInt(self, bytesToConvert):
        # "big" = MSB at beginning
        intVal = int.from_bytes(bytesToConvert, 'big')
        return intVal
